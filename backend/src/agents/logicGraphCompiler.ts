...
function pageInsight(page: LogicPage): string {
...
function continueChoice(id: string, text: string, nextNode: string): Choice {
  return {
    logic_choice_id: id,
    text: `${id}: ${text}`,
    next_node: nextNode,
    stat_delta: zeroDelta(),
    stat_reason: "继续按显式 if/else 路由推进。",
    logic_delta: {},
  };
}

function compileDecisionNode(graph: LogicGraphDocument, page: LogicPage): StoryNode {
  ...
  return {
    ...
    insight: pageInsight(page),
    logic_page_role: "node",
    logic_source_id: page.id,
  };
}

function compileResultNode(graph: LogicGraphDocument, page: LogicPage): StoryNode {
  ...
  return {
    ...
    image_prompt: null,
    has_image: false,
    choices: [continueChoice("C1_continue", label, RESULT_RETURN_SENTINEL)],
    insight: pageInsight(page),
...
function compileWarningNode(page: LogicPage): StoryNode {
  return {
    type: "warning",
    scene_text: pageText(page, `Warning page for ${page.title}.`),
    image_prompt: null,
    has_image: false,
    choices: [continueChoice("C1_warning_return", "知道了，回到刚才被打断的路线", WARNING_RETURN_SENTINEL)],
    insight: pageInsight(page),
...
export function compileLogicGraphToStoryDocument(
  ...
  for (const page of Object.values(graph.pages)) {
    if (page.role === "node") {
      nodes[page.id] = compileDecisionNode(graph, page);
    } else if (page.role === "result") {
      nodes[page.id] = compileResultNode(graph, page);
    } else if (page.role === "warning") {
      nodes[page.id] = compileWarningNode(page);
    }
  }
  ...
  for (const ending of Object.values(graph.endings)) {
    endings[ending.id] = {
      ...
      has_image: false,
      tone: ending.tone,
      insight: "This ending is selected by explicit if/else conditions over graph state, variables, and node IDs.",
